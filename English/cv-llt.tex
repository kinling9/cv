...
%% so a clone without the image still compiles -- just without it.
...
%% Local settings. personal.tex is gitignored; personal-example.tex is the
%% committed template used when it is absent.
\IfFileExists{personal.tex}{\input{personal}}{\input{personal-example}}

\leftheader{%
  {\LARGE\bfseries\sffamily Min Wei}

  % Defined in personal.tex; each field is omitted when its value is empty.
